...
def build_model_file(idx, step, x_min, x_max):
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "flowstar-2.1.0/attitude.model")
    ...
    for i_line in range(len(lines_ori)):
        if "gnuplot interval" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("gnuplot interval ")[0] + "gnuplot interval w{}, varphi{}\n".format(idx + 1, idx + 1)
        elif "output" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("output ")[0] + "output attitude_w{}_varphi{}_step{}\n".format(idx + 1, idx + 1, step)
        elif "w1 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("w1 in ")[0] + "w1 in [{}, {}]\n".format(x_min[3], x_max[3])
        elif "w2 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("w2 in ")[0] + "w2 in [{}, {}]\n".format(x_min[4], x_max[4])
        elif "w3 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("w3 in ")[0] + "w3 in [{}, {}]\n".format(x_min[5], x_max[5])
        elif "varphi1 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("varphi1 in ")[0] + "varphi1 in [{}, {}]\n".format(x_min[6], x_max[6])
        elif "varphi2 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("varphi2 in ")[0] + "varphi2 in [{}, {}]\n".format(x_min[7], x_max[7])
        elif "varphi3 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("varphi3 in ")[0] + "varphi3 in [{}, {}]\n".format(x_min[8], x_max[8])
        elif "u0 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("u0 in ")[0] + "u0 in [{}, {}]\n".format(x_min[0], x_max[0])
        elif "u1 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("u1 in ")[0] + "u1 in [{}, {}]\n".format(x_min[1], x_max[1])
        elif "u2 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("u2 in ")[0] + "u2 in [{}, {}]\n".format(x_min[2], x_max[2])
        else:
            continue
    model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),  \
        "flowstar-2.1.0/attitude_w{}_varphi{}_step{}.model".format(idx + 1, idx + 1, step))
...
def run_model_file(idx, step):
    ...
    plt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),  \
        "outputs/attitude_w{}_varphi{}_step{}.plt".format(idx + 1, idx + 1, step))
...
def flowstar(step, x_min, x_max):
    ...
    for idx in range(3):
        x_min[3 + idx], x_max[3 + idx], x_min[6 + idx], x_max[6 + idx] = read_plt_file(idx, step)
    return x_min[3:], x_max[3:]
