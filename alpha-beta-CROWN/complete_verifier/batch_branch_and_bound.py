...
def relu_bab_parallel(net, domain, x, use_neuron_set_strategy=False, refined_lower_bounds=None,
    ...
    if arguments.Config["solver"]["alpha-crown"]["no_joint_opt"]:
        ...
        print("refined_lower_bounds is None or refined_upper_bounds is None: ...., slope, ... = net.build_the_model(....)")
    ...
    if global_lb > decision_thresh:
        return global_lb, global_ub, [[time.time()-start, global_lb]], 0
    ################### There can be a y s.t. c[y] (=-1) * u[y] <= decision_thresh(0) #########
    ################### But this does not mean that this global_lb should not be returned #######
    ################### Because for classifier, the outputs are all positive. Once global_lb <= 0, the bounds should be refined ########
    ################### However, for controller, the outputs do not have to be positive. It should be fine to return those global_lb, global_ub #############
    if global_lb <= global_ub:
        return global_lb, global_ub, [[time.time()-start, global_lb]], 0
    if True:
        print("global_lb {} > global_ub {} ".format(global_lb, decision_thresh))
