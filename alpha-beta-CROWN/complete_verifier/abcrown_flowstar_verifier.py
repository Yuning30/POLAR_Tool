...
def build_model_file(exp_name, plt_name, idx, step, ux_min, ux_max):
    model_name = f"{exp_name}"
    #for i in idx[:-1]:
    model_name = model_name + f"_x{idx[0]}"
    model_name = model_name + f"_x{idx[-1]}"
    plt_name = plt_name + f"_x{idx[0]}"
    plt_name = plt_name + f"_x{idx[-1]}"
    if step > 0:
        model_name = model_name + f"_step{step}"
        plt_name = plt_name + f"_step{step}"
    ...
    for i_line in range(len(lines_ori)):
        if "gnuplot interval" in lines_ori[i_line]:
            ...
            lines_ori[i_line] = lines_ori[i_line].split("output ")[0] + f"output {plt_name}"
    ...
    with open(model_path, 'w') as f:
        for line in lines_ori:
            f.write(line)
        f.close()
    return model_name, plt_name

def run_model_file(model_name, plt_name):
...
def flowstar(exp_name, plt_name, ids, step, ux_min, ux_max):
    ux_min_tmp = {'x': {}, 'u': {}}
    ux_max_tmp = {'x': {}, 'u': {}}
    base_plt_paths = []
    for i in range(len(ids)):
        idx = ids[i]
        model_name, plt_name = build_model_file(exp_name, plt_name, idx, step, ux_min, ux_max)
        print("Built model {}".format(model_name + ".model"))
        plt_path = run_model_file(model_name, plt_name)
        print("Generated plt file at {}".format(plt_path))
        ...
        if step == 0:
            base_plt_paths.append(plt_path)
...
def main():
    ...
    for step in step_ids:
        ...
        try:
            data_min, data_max, base_plt_paths = flowstar(exp_name, plt_name, plt_ids, step, ux_min, ux_max)
