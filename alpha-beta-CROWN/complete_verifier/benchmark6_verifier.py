...
def build_model_file(idx, step, x_min, x_max):
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "flowstar-2.1.0/benchmark6.model")
    ...
    for i_line in range(len(lines_ori)):
        if "gnuplot interval" in lines_ori[i_line]:
            ...
            lines_ori[i_line] = lines_ori[i_line].split("output ")[0] + "output benchmark6_x{}_x{}_step{}\n".format(idx, idx + 1, step)
    ...
    model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),  \
        "flowstar-2.1.0/benchmark6_x{}_x{}_step{}.model".format(idx, idx + 1, step))
...
def run_model_file(idx, step):
    ...
    plt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),  \
        "outputs/benchmark6_x{}_x{}_step{}.plt".format(idx, idx + 1, step))
...
def main():
    ...
    # Load model
    model_ori = AttitudeController(os.path.join(os.path.dirname(__file__), "models/POLAR/benchmark6/nn_6_relu")).to(arguments.Config["general"]["device"])
