...
def build_model_file(idx, step, x_min, x_max):
    path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "flowstar-2.1.0/benchmark5.model")
    ...
    for i_line in range(len(lines_ori)):
        if "gnuplot interval" in lines_ori[i_line]:
            ...
            lines_ori[i_line] = lines_ori[i_line].split("output ")[0] + "output benchmark5_x{}_x{}_step{}\n".format(idx, idx + 1, step)
        ...
        elif "x2 in" in lines_ori[i_line]:
            lines_ori[i_line] = lines_ori[i_line].split("x2 in ")[0] + "x2 in [{}, {}]\n".format(x_min[3], x_max[3])
    ...
    model_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),  \
        "flowstar-2.1.0/benchmark5_x{}_x{}_step{}.model".format(idx, idx + 1, step))
...
def run_model_file(idx, step):
    ...
    plt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),  \
        "outputs/benchmark5_x{}_x{}_step{}.plt".format(idx, idx + 1, step))
...
def main():
    ...
    # Load model
    model_ori = AttitudeController(os.path.join(os.path.dirname(__file__), "models/POLAR/benchmark5/nn_5_relu")).to(arguments.Config["general"]["device"])
