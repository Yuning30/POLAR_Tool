...
def config_args():
    ...
    arguments.Config.add_argument('--network_name', type=str, default="CLF_controller_layer_num_3",
...
def main(steps = 5):
    ...
    Xs = np.zeros((steps+1, 12))
    Xs[0] = [x0_min, x0_max, x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max, x5_min, x5_max]

    for step in range(steps):
        us_min = ctrl_input_bound(model_lb, [x0_min, x1_min, x2_min, x3_min, x4_min, x5_min], [x0_max, x1_max, x2_max, x3_max, x4_max, x5_max])
        us_max = -ctrl_input_bound(model_ub, [x0_min, x1_min, x2_min, x3_min, x4_min, x5_min], [x0_max, x1_max, x2_max, x3_max, x4_max, x5_max])
        u0_min = (us_min[0] - model_lb.output_offset) * model_lb.output_scale
        u0_max = (us_max[0] - model_ub.output_offset) * model_ub.output_scale
        u1_min = (us_min[1] - model_lb.output_offset) * model_lb.output_scale
        u1_max = (us_max[1] - model_ub.output_offset) * model_ub.output_scale
        ...
        x3_range = [float(i) for i in lines[3].split(' ')]
        x4_range = [float(i) for i in lines[4].split(' ')]
        x5_range = [float(i) for i in lines[5].split(' ')]
        x0_min = x0_range[0]
        x0_max = x0_range[1]
        x1_min = x1_range[0]
        x1_max = x1_range[1]
        ...
        x5_min = x5_range[0]
        x5_max = x5_range[1]
        Xs[step+1] = [x0_min, x0_max, x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max, x5_min, x5_max]

    fig, ax = plt.subplots()
    for bound in Xs:
        print(bound)
        x0_min, x0_max, x1_min, x1_max, x2_min, x2_max, x3_min, x3_max, x4_min, x4_max, x5_min, x5_max = bound
        ax.plot([x0_min]*2, [x3_min, x3_max], 'b')
        ax.plot([x0_max]*2, [x3_min, x3_max], 'b')
        ax.plot([x0_min, x0_max], [x3_min]*2, 'b')
        ax.plot([x0_min, x0_max], [x3_max]*2, 'b')
