...
int main(int argc, char *argv[])
{
	...
	*/
	...
	ifstream dynamics_file("./results/learned_dynamics/Marvelgymnasium_lalo-v1/model.txt");
	...
	*/
	 ...
	 */
	int steps = 100;
	Interval init_x1(1.15, 1.25), init_x2(1.00, 1.10), init_x3(1.45, 1.55), init_x4(2.35, 2.45), init_x5(0.95, 1.05), init_x6(0.05, 0.15), init_x7(0.40, 0.50);
	...
	vector<vector<double>> unsafe_set = {{-3.30, 1.25}, {-3.45, 1.10}, {-3.00, 1.55}, {-2.10, 2.45}, {-3.50, 1.05}, {-4.40, 1.05}, {-4.05, 0.50}};
	for (int iter = 0; iter < steps; ++iter)
	{
		...
		tmv_input.tms.push_back(initial_set.tmvPre.tms[6]);
		...
		initial_set.tmvPre.tms[x8_id] = tmv_output.tms[0];
		...
		if (result.status == COMPLETED_SAFE || result.status == COMPLETED_UNSAFE || result.status == COMPLETED_UNKNOWN)
		{
			...
			for (int i = 0; i < 7; ++i)
			{
				reached_set.push_back({inter_box[i].inf(), inter_box[i].sup()});
			}
			double step_safe_loss = calculate_safe_loss(reached_set, unsafe_set);
			if (iter == 0 && step_safe_loss < 0.0006)
			{
				step_safe_loss = 0.0;
			}
			safe_loss += step_safe_loss;
			// cout << "iter " << iter << " " << calculate_safe_loss(reached_set, unsafe_set) << endl;
			if (plot || print_safe_sets)
			{
				...
				cout << inter_box[6].inf() << " " << inter_box[6].sup() << "\n";
			}
