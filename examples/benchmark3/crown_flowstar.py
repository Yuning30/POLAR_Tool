...
def config_args():
    ...
    arguments.Config.add_argument('--network_name', type=str, default="nn_3_relu",
...
def main(steps = 5):
    ...
    Xs[0] = [x0_min, x0_max, x1_min, x1_max]

    for step in range(steps):
        us_min = ctrl_input_bound(model_lb, [x0_min, x1_min], [x0_max, x1_max])
        us_max = -ctrl_input_bound(model_ub, [x0_min, x1_min], [x0_max, x1_max])
        ...
        x1_min = x1_range[0]
        x1_max = x1_range[1]
        Xs[step+1] = [x0_min, x0_max, x1_min, x1_max]
...
if __name__ == "__main__":
    config_args()
    main(60)        # Flowstar error at step 27 because of too large input range
